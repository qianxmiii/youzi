...
def _migrate_columns(conn: sqlite3.Connection) -> None:
    ...
    if "settlement_method" not in cols:
        conn.execute(f"ALTER TABLE {TABLE_NAME} ADD COLUMN settlement_method TEXT")
    if "settlement_day" not in cols:
        conn.execute(f"ALTER TABLE {TABLE_NAME} ADD COLUMN settlement_day INTEGER")
