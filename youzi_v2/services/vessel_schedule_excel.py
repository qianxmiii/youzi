...
def parse_excel_rows(
    ...
    if "vessel_voyage" not in header_index:
        wb.close()
        return {}, [{"row": 1, "message": "缺少「船名航次」列"}]
    if "port_name" not in header_index:
        wb.close()
        return {}, [{"row": 1, "message": "缺少「港口」列"}]

    grouped: dict[str, dict[str, Any]] = defaultdict(lambda: {"port_calls": [], "notes": ""})
    ...
    for row in rows_iter:
        ...
        if notes:
            grouped[vessel_voyage]["notes"] = notes
        grouped[vessel_voyage]["port_calls"].append(
            {
                "sequence": _cell_int(pick("sequence"), len(grouped[vessel_voyage]["port_calls"]) + 1),
                "port_name": port_name,
...
def build_template_bytes() -> bytes:
    headers = ["船名航次", "序号", "港口", "ETA", "ATA", "ETD", "ATD", "备注"]
    samples = [
        ["CSCL BOHAI SEA/076E", 1, "Yantian", "", "", "2026-05-10 08:00", "2026-05-11 12:00", ""],
        ["CSCL BOHAI SEA/076E", 2, "Kaohsiung", "2026-05-12 10:00", "2026-05-12 11:30", "2026-05-13 06:00", "2026-05-13 08:00", ""],
        ["CSCL BOHAI SEA/076E", 3, "Long Beach", "2026-05-29 16:30", "", "2026-06-01 10:00", "", ""],
    ]
    wb = Workbook()
    ws = wb.active
    ws.title = "船期"
