...
from youzi_v2.db.shipment_sla import ALERT_WAREHOUSE_NO_DEPARTURE, SLA_EXCLUDED_CHANNEL_CODES
...
def test_scan_skips_excluded_fc_channel(tmp_path: Path) -> None:
    db = Database(tmp_path / "sla_scan_fc.db")
    ensure_shipments_schema(db.conn)
    ensure_alerts_schema(db.conn)
    ship_id = str(uuid.uuid4())
    alert_id = str(uuid.uuid4())
    ...
    _insert_shipment(
        ...
        ship_id=ship_id,
        shipment_no="SN-FC-1",
        channel_code=next(iter(SLA_EXCLUDED_CHANNEL_CODES)),
        warehouse_entry_time="2026-06-01",
    )
    with db.lock:
        db.conn.execute(
