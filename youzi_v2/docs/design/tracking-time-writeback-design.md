...
5. `signed_time` 从 `signed for` 或首字母大写 `Delivered` 节点识别；当签收节点事件日期和预计送仓日期不一致时，进入人工审批并询问是否采用预计送仓时间作为签收时间。
...
首版人工审批重点只放在签收时间冲突。冲突时系统不默认采用签收节点事件时间，而是推荐采用业务上更可信的预计送仓时间：

```text
签收节点事件日期 != 最新预计送仓日期
审批问题：是否采用预计送仓时间作为签收时间？
```
...
- 签收节点时间使用签收节点轨迹事件时间，不从文本中解析日期。
- 如果同时存在预计送仓时间和签收节点，且两者日期不一致，签收时间审批的推荐值为预计送仓时间，对照值为签收节点事件时间。
...
| `candidate_value` | 系统推荐写入正式字段的候选时间 |
| `compare_value` | 审批时用于对照的时间，例如签收节点事件时间 |
| `recommended_source` | 推荐值来源，例如 `expected_delivery_time`、`signed_track_time` |
| `compare_source` | 对照值来源，例如 `signed_track_time` |
| `source_track_id` | 推荐值来源内部轨迹 ID |
| `source_track_time` | 推荐值来源轨迹事件时间 |
| `compare_source_track_id` | 对照值来源内部轨迹 ID |
| `compare_source_track_time` | 对照值来源轨迹事件时间 |
...
signed_time_compare_value
signed_time_recommended_source
signed_time_compare_source
signed_time_source_track_id
signed_time_compare_source_track_id
...
识别到 signed for 或 Delivered:
  运单状态可标记为已签收
  signed_track_time = 签收节点 track_event_time
```

正式 `signed_time` 回写规则：

```text
如果存在 signed_track_time:
  如果不存在 expected_delivery_time:
    signed_time = signed_track_time
    signed_time_candidate = signed_track_time
    recommended_source = signed_track_time
    review_status = auto_confirmed

  如果存在 expected_delivery_time:
    如果 same_date(signed_track_time, expected_delivery_time):
      signed_time = expected_delivery_time
      signed_time_candidate = expected_delivery_time
      compare_value = signed_track_time
      recommended_source = expected_delivery_time
      compare_source = signed_track_time
      review_status = auto_confirmed
    否则:
      signed_time 不覆盖
      signed_time_candidate = expected_delivery_time
      ...
      review_reason = 签收节点事件日期与预计送仓日期不一致，需确认是否采用预计送仓时间作为签收时间
```
...
def recalculate_tracking_time_fields(shipment_id):
    ...
    apply_signed_time_with_review(
        shipment_id,
        signed_track_time=candidates["signed_time"],
...
| 动作 | 结果 |
| --- | --- |
| 采用预计送仓时间 | `signed_time = expected_delivery_time`，`review_status = manual_approved` |
| 使用签收节点时间 | `signed_time = signed_track_time`，`review_status = manual_approved` |
...
signed_track_time = 2026-06-14
signed_time_candidate = 2026-06-14
signed_time_compare_value = 2026-06-14
signed_time_recommended_source = expected_delivery_time
signed_time_compare_source = signed_track_time
...
signed_track_time = 2026-06-28
signed_time_candidate = 2026-06-15
signed_time_compare_value = 2026-06-28
signed_time_recommended_source = expected_delivery_time
signed_time_compare_source = signed_track_time
signed_time 不自动覆盖
signed_time_review_status = pending_review
signed_time_review_question = 是否采用预计送仓时间 2026-06-15 作为签收时间？
...
```text
signed_time = 2026-06-15
signed_time_review_status = manual_approved
```
...
首版取最新签收节点事件时间作为 `signed_track_time`。

如果旧候选已经进入人工审批但尚未处理，后续出现新的签收轨迹时，可以更新 `signed_track_time`、对照值和审批问题，并保留旧审批记录。
