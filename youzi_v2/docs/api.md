...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/` | 列表（筛选、分页；含 `groupId`、`groupNo`、`groupType`、`hasGroup`；响应 `groups` 数组） |
| GET | `/{item_id}` | 单条详情 |
| POST | `/` | 新建 |
| PUT | `/{item_id}` | 更新 |
| DELETE | `/{item_id}` | 删除 |
...
| GET | `/api/v1/shipment-groups` | 分组列表（`search`、`groupType` 表示**包含**该类型、`paymentStatus`、`customer`） |
| POST | `/api/v1/shipment-groups` | 新建分组（`groupNo` 可省略，自动生成 `GYYYYMMDDnnn`） |
...
| GET | `/api/v1/shipment-group-notifications` | 顶栏/首页未读提醒（`items` + `unreadCount`） |
...
curl "http://127.0.0.1:3001/api/v1/shipments?groupNo=G20260618001&limit=20"
```
