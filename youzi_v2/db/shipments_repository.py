...
def _row_to_api(row: sqlite3.Row) -> dict[str, Any]:
    return {
        ...
        "latestTrackingTime": row["latest_tracking_time"],
        "latestTrackingDesc": row["latest_tracking_desc"],
    ...
    def list_rows(
        ...
        *,
        ...
        if search and search.strip():
            q = f"%{search.strip()}%"
            conditions.append(
                "(shipment_no LIKE ? OR customer LIKE ? OR customer_no LIKE ? "
        ...
        with self._database.lock:
            if count is None:
                count = self._conn.execute(
                    "SELECT COUNT(*) AS c FROM tracking_logs WHERE shipment_no = ?",
                    (sn,),
                ).fetchone()["c"]
            self._conn.execute(
                f"""
